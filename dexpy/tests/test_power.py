...
class TestPower(TestCase):
    ...
    def test_large_power(self):
        ...
        combos = [''.join(combo) for order in range(1, max_order) for combo in itertools.combinations([dexpy.model.Term.valid_vars[var] for var in range(0, factor_count)], order)]
        model_str = "1 + " + " + ".join(combos)

        response_data = []

        design = dexpy.design.Design(factor_data, response_data)
        model = dexpy.model.LinearModel.from_string(model_str)
        X = design.create_model_matrix(model)
