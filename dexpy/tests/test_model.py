...
class TestTerm(TestCase):
    ...
    def test_parse_double_prime_term(self):
        ...
        term = dexpy.Term.from_string('P"^2')
        self.assertEqual(2, term.powers[64])

    def test_parse_intercept(self):

        term = dexpy.Term.from_string('1')
        self.assertEqual(1, term.coefficient)
        self.assertEqual(0, len(term.powers))

    def test_always_positive(self):

        self.assertTrue(dexpy.Term.from_string('A^2').always_positive())
        self.assertFalse(dexpy.Term.from_string('A^3').always_positive())
        self.assertTrue(dexpy.Term.from_string('A^4').always_positive())
...
class TestLinearModel(TestCase):
    ...
    def test_parse_model(self):
        ...
        model = dexpy.LinearModel.from_string("1+A+ 2B +C+AB + 4.1*AbC + 10A*C + A^2 + B^2 + A^2B + AB^2")
